...
import {
  Plus, Edit, Trash2, Layers, ImageIcon,
  Download, Upload, MoreHorizontal,
} from "lucide-react";
import { useCollections, useDeleteCollection, type Collection } from "@/lib/queries";
...
export function CollectionsTable({ storeId }: { storeId: string }) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      notification.success("Collection deleted", "The collection has been deleted successfully");
    } catch (error) {
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">{collections.length} total</p>
        </div>
      ...
      <DataTable
        ...
        builtinBulkActions={{
          onBulkDelete: async (items) => {
            ...
            try {
              await Promise.all(items.map((c) => deleteCollection.mutateAsync((c as Collection).id)));
              notification.success("Collections deleted", `${items.length} collections have been deleted successfully`);
            } catch (error) {
              notification.error("Failed to delete", error instanceof Error ? error.message : "Failed to delete some collections");
            }
          },
        }}
        actions={(item) => {
